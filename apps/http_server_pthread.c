/*
 * http_server.c - Multi-threaded HTTP Web Server using LIBPTHREAD
 *
...
void signal_handler(int signo) {
    ...
    pthread_cond_broadcast(&g_server.queue.not_empty);
    pthread_cond_broadcast(&g_server.queue.not_full);
}
...
int server_init(int port, int num_workers, const char *www_root) {
    ...
    pthread_rwlock_init(&g_server.stats_lock, NULL);
...
int main(int argc, char *argv[])
{
    ...
    if (pthread_create(&acceptor, NULL, acceptor_thread, NULL) != 0) {
    ...
    pthread_rwlock_destroy(&g_server.stats_lock);
