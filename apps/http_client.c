/*
 * http_client.c - Multi-threaded HTTP Client using LIBAPTH
 *
...
void *client_worker(void *arg) {
    ...
    apth_barrier_wait(&g_config.stats.start_barrier);
...
APTH_MAIN_BEGIN(argc, argv)
{
    ...
    apth_mutex_init(&g_config.stats.mutex, NULL);
    apth_barrier_init(&g_config.stats.start_barrier, NULL, g_config.num_threads + 1);

    apth_t threads[g_config.num_threads];
    int thread_ids[g_config.num_threads];

    for (int i = 0; i < g_config.num_threads; i++) {
        thread_ids[i] = i;
        if (apth_create(&threads[i], NULL, client_worker, &thread_ids[i]) != 0) {
    ...
    apth_mutex_destroy(&g_config.stats.mutex);
    apth_barrier_destroy(&g_config.stats.start_barrier);
