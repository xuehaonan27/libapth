...
// context.
static inline bool __apth_is_synchronous_fault(int sig)
{
                 ...
                 (sig, act, oldact))
{
    ...
    // Synchronous fault signals (SIGSEGV, SIGBUS, etc.) are generated by
    // hardware and delivered to the faulting thread.  They cannot be
    // simulated purely in software, so also register with the kernel.
    // Add SA_ONSTACK so the handler can run on the alternate signal stack
    // when an M:N thread has overflowed its own stack.
    if (__apth_is_synchronous_fault(sig) && act != NULL)
    {
        struct sigaction kernel_sa = *act;
        kernel_sa.sa_flags |= SA_ONSTACK;
        return apth_func_raw(sigaction)(sig, &kernel_sa, NULL);
    }
...
APTH_DEFINE_HOOK(int, sigwait, (const sigset_t *set, int *sigp), (set, sigp))
{
    ...
    // Block the waited signals in sigmask so apth_deliver_pending_signals()
    // won't deliver them via handler while we're waiting.  POSIX mandates
    // that signals in the sigwait set should already be blocked; we enforce
    // it here defensively.
    sigset_t saved_mask = self->sigmask;
    for (int sig = 1; sig < APTH_NSIG; sig++)
    {
        if (sigismember(set, sig))
            sigaddset(&self->sigmask, sig);
    }
    ...
    // Loop because apth_sched_wake_thread (called by apth_kill) may move
    // us from the waiting queue to the wake stack without resolving the
    // SIGS event.  In that case, check sigpending directly.
    for (;;)
    {
        struct apth_event_st ev = EVENT_SIGS(set, sigp);
        apth_wait_event(&ev);

        if (ev.ev_status == APTH_EV_STATUS_OCCURRED)
        {
            self->sigmask = saved_mask;
            return 0;
        }
